...
{
	const char *TAG = "memScreen";
	const char *st = lv_table_get_cell_value(table, row_, col_);
	string epcFromTable = st;
	int n = Inventory(false);
	string tidString;
	/*refresh ui*/
	lv_label_set_text(ui_chipTypeLabel, "");
	lv_label_set_text(ui_epcSizeLabel, "");
	lv_label_set_text(ui_pwdSizeLabel, "");
	lv_label_set_text(ui_userSizeLabel, "");
	lv_label_set_text(ui_totatMemoryLabel, "");
	...
	for (uint8_t i = 0; i < n; i++)
	{
		...
		if (epcFromTable == epcFromScan)
		{
			if (GetTID(sd.epc, sd.epclen, tid, &tidlen))
			{
				...
				ESP_LOGI(TAG, "tid found\n");
				tidString = bytes2hex(tid, tidlen);
				GetTagInfo(tid, &ti);
				/*tid found, populate ui*/
				lv_label_set_text(ui_chipTypeLabel, ti.chip);
				lv_label_set_text(ui_epcSizeLabel, intToString(ti.epclen));
				lv_label_set_text(ui_pwdSizeLabel, intToString(ti.pwdlen));
				lv_label_set_text(ui_userSizeLabel, intToString(ti.userlen));
				uint16_t totalmem = (ti.epclen) + (ti.userlen) + ti.tidlen + (ti.pwdlen * 2) + 4;
				lv_label_set_text(ui_totatMemoryLabel, intToString(totalmem));
				/*read EPC & TID*/
				...
				for (size_t i = 0; i < numSubstrings; ++i)
				{
					...
					lv_table_set_cell_value_fmt(table1, 1, i + 1, "%s", substrings[i].c_str());
					lv_table_set_cell_value_fmt(table1, 2, i + 1, "%s", substrings_[i].c_str());
				...
				{
					printf("usrlen<8\n");
					lv_table_set_row_cnt(table1, 2 + 1);
					lv_table_set_cell_value(table1, 2 + 1, 0, "USR(7)");
				}
				else if ((ti.userlen / 2) >= 8 && (ti.userlen / 2) <= 32)
				{
					printf("usrlen 8-32\n");
					lv_table_set_row_cnt(table1, 2 + 4);
					lv_table_set_cell_value(table1, 2 + 1, 0, "USR(7)");
					lv_table_set_cell_value(table1, 2 + 2, 0, "(8-15)");
					lv_table_set_cell_value(table1, 2 + 3, 0, "(16-23)");
					lv_table_set_cell_value(table1, 2 + 4, 0, "(24-31)");
				}
				/*read user memory*/
				for (uint8_t j = 0; j < ti.userlen / 2; j++)
				{
					if (ReadMemWord(sd.epc, sd.epclen, mem_data, j))
					{
						if (j < 8)
						{
							lv_table_set_cell_value_fmt(table1, 2 + 1, j + 1, "% 02x % 02x", mem_data[0], mem_data[1]);
							printf("j : %d, userData %2x%2x\n", j, mem_data[0], mem_data[1]);
							fflush(stdout);
							/*create new user row TODO*/
							// lv_table_set_cell_value_fmt(table1, 4, j + 8, "% 02x % 02x", mem_data[0], mem_data[1]);
						}
						else if (j >= 8 && j < 16)
						{

							lv_table_set_cell_value_fmt(table1, 4, j - 7, "% 02x % 02x", mem_data[0], mem_data[1]);
						}
						else if (j >= 16 && j < 24)
						{

							lv_table_set_cell_value_fmt(table1, 5, j - 15, "% 02x % 02x", mem_data[0], mem_data[1]);
						}
						else if (j >= 24 && j < 32)
						{

							lv_table_set_cell_value_fmt(table1, 6, j - 23, "% 02x % 02x", mem_data[0], mem_data[1]);
						}

						memset(mem_data, 0x00, sizeof(mem_data));
						// printf("data after memcpy %02x %02x", mem_data[0], mem_data[1]);
						// fflush(stdout);
					}
					else
					{
						ESP_LOGI(TAG, "Failed to read user data");
						/*set user cells to null in UI*/
						// lv_table_set_cell_value_fmt(table1, 3, j + 1, "% s % s", NULL, "");
						break;
					}
				}
			}
...
bool WriteEpcWord_u(unsigned char *epc, unsigned char epclen, unsigned char *data, unsigned char windex)
{
	...
	{
		lv_label_set_text_fmt(ui_memScreenNotifyLabel, "EPC data written at word%u.", col_ - 1);
	...
	{
		lv_label_set_text(ui_memScreenNotifyLabel, "Failed to write EPC.");
		vTaskDelay(2000 / portTICK_PERIOD_MS);
		lv_label_set_text(ui_memScreenNotifyLabel, "");
		return 0;
	}
...
{
	const char *txtAreaData = lv_textarea_get_text(ui_memScrTextarea);
	...
	if (strlen(txtAreaData) == 4)
	{
		...
		case 1:
			...
			{
				lv_table_set_cell_value(table1, row_, col_, txtAreaData);
			}
			break;
		case 2:
			/*HANDLE TID DATA*/
		...
		case 4:
			...
			if (WriteMemWord_u(EPCNUM_g, epclen_g, mem_data_, col_ - 1))
			{
				lv_table_set_cell_value(table1, row_, col_ - 8, txtAreaData);
			}
			break;
		case 5:
			/*USR16-24*/
			...
			if (WriteMemWord_u(EPCNUM_g, epclen_g, mem_data_, col_ - 1))
			{
				lv_table_set_cell_value(table1, row_, col_ - 8 * 2, txtAreaData);
			}
			break;
		case 6:
			/*USR25-31*/
			...
			if (WriteMemWord_u(EPCNUM_g, epclen_g, mem_data_, col_ - 1))
			{
				lv_table_set_cell_value(table1, row_, col_ - 8 * 3, txtAreaData);
			}
			break;
		default:
			/*INVALID ROW*/
		...
		lv_textarea_set_text(ui_memScrTextarea, "");
	}
	else
	{
		printf("data size invalid %d\n", strlen(txtAreaData));
		fflush(stdout);
		lv_label_set_text(ui_memScreenNotifyLabel, "invalid data");
		vTaskDelay(2000 / portTICK_PERIOD_MS);
		lv_textarea_set_text(ui_memScrTextarea, "");
		lv_label_set_text(ui_memScreenNotifyLabel, "");
	}
...
void mem_scr_write(lv_event_t *e)
{
	...
	{
		ESP_LOGI(TAG, "TASK ALREADY RUNNING");
	}
}
...
void onChangeASCII(lv_event_t *e)
{
	if (!onHexTextarea)
	{
		...
		passAscii = lv_textarea_get_text(ui_passASCIItxtarea);
		const char *passHex = ascii2hex(passAscii);
		lv_textarea_set_text(ui_passHEXtxtarea, passHex);
...
void onChangeHEX(lv_event_t *e)
{
	...
	if (!onAciiTextarea)
	{
		onHexTextarea = true;
		const char *passhex;
		passhex = lv_textarea_get_text(ui_passHEXtxtarea);
		const char *passascii = hex2ascii(passhex);
		lv_textarea_set_text(ui_passASCIItxtarea, passascii);
		delete[] (passascii);
		onHexTextarea = false;
	}
}
...
{
	const char *passHex_;
	passHex_ = lv_textarea_get_text(ui_passHEXtxtarea);
	int passHexLen = (strlen(passHex_) + 1) / 2;
	printf(" passHexLen %d\n", passHexLen);
	if (SetPassword(EPCNUM_g, epclen_g, (unsigned char *)passHex_, 4))
	{
		lv_label_set_text(ui_memScreenNotifyLabel, "");
		lv_label_set_text_fmt(ui_memScreenNotifyLabel, "Pass set: %s", passHex_);
	}
	else
	{
		lv_label_set_text(ui_memScreenNotifyLabel, "");
		lv_label_set_text(ui_memScreenNotifyLabel, "Pass not set");
	}
}
void pass_use(lv_event_t *e)
{
	lv_label_set_text(ui_memScreenNotifyLabel, "");
	lv_label_set_text(ui_memScreenNotifyLabel, "Pass use pressed");
}
void pass_default(lv_event_t *e)
{
	lv_label_set_text(ui_memScreenNotifyLabel, "");
	lv_label_set_text(ui_memScreenNotifyLabel, "Pass default pressed");
}
void test_btn_pressed(lv_event_t *e)
{
	lv_label_set_text(ui_memScreenNotifyLabel, "");
	lv_label_set_text(ui_memScreenNotifyLabel, "Test btn pressed");
}
void test_btn1_pressed(lv_event_t *e)
{
	lv_label_set_text(ui_memScreenNotifyLabel, "");
	lv_label_set_text(ui_memScreenNotifyLabel, "Test btn1 pressed");
}
...
void test_btn2_pressed(lv_event_t *e)
{
	// Your code here
	lv_label_set_text(ui_memScreenNotifyLabel, "");
	lv_label_set_text(ui_memScreenNotifyLabel, "Test btn2 pressed");
}
...
bool WriteMemWord_u(unsigned char *epc, unsigned char epclen, unsigned char *data, unsigned char windex)
{
	...
	{
		lv_label_set_text_fmt(ui_memScreenNotifyLabel, "User data written at word%u.", col_ - 1);
		vTaskDelay(2000 / portTICK_PERIOD_MS);
		lv_label_set_text(ui_memScreenNotifyLabel, "");
		return 1;
	}
	...
	{
		lv_label_set_text(ui_memScreenNotifyLabel, "Failed to write.");
		vTaskDelay(2000 / portTICK_PERIOD_MS);
		lv_label_set_text(ui_memScreenNotifyLabel, "");
		return 0;
	}
